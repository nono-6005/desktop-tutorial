...
└── templates/
    ├── index.html               # アプリ本体（UI + JS + IndexedDB + ブックマーク案内）
...
**index.html**
- `{{APP_NAME}}` - アプリ正式名（タイトル・見出し・meta タグ）
- `{{THEME_COLOR}}` - 16進色コード（ボタン・アクセントカラー）
- `{{APP_DESCRIPTION}}` - 説明文（meta description）
...
3. パラメータをユーザーに確認
4. templates/ から index.html / manifest.json / sw.js / README.md をテンプレート処理して生成
5. Git にコミット
6. GitHub Pages 設定の案内を表示

### index.html について

`templates/index.html` は実運用（desktop-tutorial リポジトリ）で検証済みの実装をベースにしています。含まれる機能：

- メモのリスト表示 + エディタビュー（タイトル + Markdown エディタ）
- IndexedDB 操作（作成・保存・削除）
- ダウンロード機能（Blob ベース、.md ファイル出力）
- レスポンシブ対応（768px 未満はモバイル1カラム、以上はタブレット/PC 2カラム）
- ブックマーク案内モーダル（iOS/Android/デスクトップ別の手順を表示。`beforeinstallprompt` は端末によって発火しないことがあるため、これに依存しないインストール代替導線として採用）
